...
int BLDCDriver6PWM::init() {
  ...
  params = _configure_6_pwm(pwm_frequency, dead_zone, pwmA_h,pwmA_l, pwmB_h,pwmB_l, pwmC_h,pwmC_l);
...
void BLDCDriver6PWM::set_pwm(float Ua, float Ub, float Uc) {
  ...
  // hardware specific function - depending on driver and mcu
  _write_duty_cycle_6_pwm(dc_a, dc_b, dc_c, phase_state, params);
}


// Set the phase state
// actually changing the state is only done on the next call to set_pwm, and depends
// on the hardware capabilities of the driver and MCU.
void BLDCDriver6PWM::set_phase_state(PhaseState sa, PhaseState sb, PhaseState sc) {
  phase_state[0] = sa;
  phase_state[1] = sb;
  phase_state[2] = sc;
}
