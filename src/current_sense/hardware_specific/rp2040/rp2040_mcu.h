 ...
 * To use the other ADC channels, use them via this engine. Use add_pin() to add them to the conversion, and get_last_result()
...
class RP2040ADCEngine {
  public:
    ...
    //void set_pwm_trigger(uint slice);

    bool init();
    void start();
    void stop();

    ADCResults get_last_results(); // TODO find a better API and representation for this
