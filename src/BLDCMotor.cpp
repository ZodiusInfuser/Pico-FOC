...
*/
void BLDCMotor::link_driver(BLDCDriver* _driver) {
  driver = _driver;
}
...
int  BLDCMotor::init_foc( float zero_electric_offset, Direction _sensor_direction) {
  int exit_flag = 1;
  ...
  sleep_ms(500);
  if(sensor){
    exit_flag *= align_sensor();
    // added the shaft_angle update
    sensor->update();
    _shaft_angle = shaft_angle();
  }else {
...
int BLDCMotor::align_current_sense() {
  int exit_flag = 1; // success

  SIMPLEFOC_DEBUG("MOT: Align current sense.");

  // align current sense and the driver
  exit_flag = current_sense->driver_align(voltage_sensor_align);
  if(!exit_flag){
...
int BLDCMotor::align_sensor() {
  int exit_flag = 1; //success
  SIMPLEFOC_DEBUG("MOT: Align sensor.");

  // check if sensor needs zero search
  if(sensor->needs_search()) exit_flag = absolute_zero_search();
  ...
  if(!_isset(sensor_direction)){
    ...
    for (int i = 0; i <=500; i++ ) {
      ...
      set_phase_voltage(voltage_sensor_align, 0,  angle);
	    sensor->update();
      sleep_ms(2);
    }
    // take and angle in the middle
    sensor->update();
    float mid_angle = sensor->get_angle();
    ...
    sensor->update();
    float end_angle = sensor->get_angle();
  ...
  if(!_isset(zero_electric_angle)){
    ...
    set_phase_voltage(voltage_sensor_align, 0,  _3PI_2);
    sleep_ms(700);
    // read the sensor
    sensor->update();
    // get the current zero electric angle
    zero_electric_angle = 0;
    zero_electric_angle = electrical_angle();
    //zero_electric_angle =  _normalize_angle(_electrical_angle(sensor_direction*sensor->getAngle(), pole_pairs));
    sleep_ms(20);
    //if(monitor_port){
...
// - to the index
int BLDCMotor::absolute_zero_search() {
  ...
  _shaft_angle = 0;
  while(sensor->needs_search() && _shaft_angle < _2PI){
  ...
  //if(monitor_port){
  //  if(sensor->needs_search()) SIMPLEFOC_DEBUG("MOT: Error: Not found!");
  //  else SIMPLEFOC_DEBUG("MOT: Success!");
  //}
  return !sensor->needs_search();
}

// Iterative function looping FOC algorithm, setting Uq on the Motor
// The faster it can be run the better
void BLDCMotor::loop_foc() {
  ...
  _electrical_angle = electrical_angle();
  switch (torque_controller) {
    ...
    case TorqueControlType::dc_current:
      ...
      current.q = current_sense->get_dc_current(_electrical_angle);
      // filter the value values
      current.q = LPF_current_q(current.q);
      // calculate the phase voltage
      voltage.q = PID_current_q(current_sp - current.q);
      // d voltage  - lag compensation
      if(_isset(phase_inductance)) voltage.d = _constrain( -current_sp*_shaft_velocity*pole_pairs*phase_inductance, -voltage_limit, voltage_limit);
      else voltage.d = 0;
      break;
    case TorqueControlType::foc_current:
      if(!current_sense) return;
      // read dq currents
      current = current_sense->get_foc_currents(_electrical_angle);
      // filter values
      current.q = LPF_current_q(current.q);
  ...
  set_phase_voltage(voltage.q, voltage.d, _electrical_angle);
}
...
void BLDCMotor::move(float new_target) {
  ...
  if( controller!=MotionControlType::angle_openloop && controller!=MotionControlType::velocity_openloop ) 
    _shaft_angle = shaft_angle(); // read value even if motor is disabled to keep the monitoring updated but not in openloop mode
  // get angular velocity  TODO the velocity reading probably also shouldn't happen in open loop modes?
  _shaft_velocity = shaft_velocity(); // read value even if motor is disabled to keep the monitoring updated
  ...
  if (_isset(KV_rating)) voltage_bemf = _shaft_velocity/KV_rating/_RPM_TO_RADS;
  ...
  switch (controller) {
    case MotionControlType::torque:
      if(torque_controller == TorqueControlType::voltage){ // if voltage torque control
        ...
        else voltage.d = _constrain( -target*_shaft_velocity*pole_pairs*phase_inductance, -voltage_limit, voltage_limit);
      }else{
    ...
    case MotionControlType::angle:
      ...
      shaft_velocity_sp = P_angle( shaft_angle_sp - _shaft_angle );
      // calculate the torque command - sensor precision: this calculation is ok, but based on bad value from previous calculation
      current_sp = PID_velocity(shaft_velocity_sp - _shaft_velocity); // if voltage torque control
      ...
      if(torque_controller == TorqueControlType::voltage){
        ...
        // set d-component (lag compensation if known inductance)
        if(!_isset(phase_inductance)) voltage.d = 0;
        else voltage.d = _constrain( -current_sp*_shaft_velocity*pole_pairs*phase_inductance, -voltage_limit, voltage_limit);
      }
      break;
    case MotionControlType::velocity:
      ...
      current_sp = PID_velocity(shaft_velocity_sp - _shaft_velocity); // if current/foc_current torque control
...
void BLDCMotor::set_phase_voltage(float Uq, float Ud, float angle_el) {
  ...
  switch (foc_modulation)
  {
    case FOCModulationType::Trapezoid_120 :
      ...
      sector = 6 * (_normalize_angle(angle_el + _PI_6 ) / _2PI); // adding PI/6 to align with other modes
      ...
      if(trap_120_map[sector][0]  == _HIGH_IMPEDANCE){
        ...
        driver->set_phase_state(PhaseState::PHASE_OFF, PhaseState::PHASE_ON, PhaseState::PHASE_ON); // disable phase if possible
      }else if(trap_120_map[sector][1]  == _HIGH_IMPEDANCE){
        Ua = trap_120_map[sector][0] * Uq + center;
        Ub = center;
        Uc = trap_120_map[sector][2] * Uq + center;
        driver->set_phase_state(PhaseState::PHASE_ON, PhaseState::PHASE_OFF, PhaseState::PHASE_ON);// disable phase if possible
      }else{
        Ua = trap_120_map[sector][0] * Uq + center;
        Ub = trap_120_map[sector][1] * Uq + center;
        Uc = center;
        driver->set_phase_state(PhaseState::PHASE_ON, PhaseState::PHASE_ON, PhaseState::PHASE_OFF);// disable phase if possible
      }
    ...
    case FOCModulationType::Trapezoid_150 :
      // see https://www.youtube.com/watch?v=InzXA7mWBWE Slide 8
      // determine the sector
      sector = 12 * (_normalize_angle(angle_el + _PI_6 ) / _2PI); // adding PI/6 to align with other modes
      // centering the voltages around either
      // modulation_centered == true > driver.voltage_limit/2
      // modulation_centered == false > or Adaptable centering, all phases drawn to 0 when Uq=0
      center = modulation_centered ? (driver->voltage_limit)/2 : Uq;
      ...
      if(trap_150_map[sector][0]  == _HIGH_IMPEDANCE){
        ...
        Ub = trap_150_map[sector][1] * Uq + center;
        Uc = trap_150_map[sector][2] * Uq + center;
        driver->set_phase_state(PhaseState::PHASE_ON, PhaseState::PHASE_ON, PhaseState::PHASE_ON); // enable all phases
      }
    ...
    case FOCModulationType::SpaceVectorPWM :
      ...
      if(Ud){ // only if Ud and Uq set
        ...
        // only necessary if using _sin and _cos - approximation functions
        angle_el = _normalize_angle(angle_el + atan2(Uq, Ud));
...
float BLDCMotor::velocity_openloop(float target_velocity){
  ...
  _shaft_angle = _normalize_angle(_shaft_angle + target_velocity*Ts);
  // for display purposes
  _shaft_velocity = target_velocity;
  ...
  set_phase_voltage(Uq,  0, _calc_electrical_angle(_shaft_angle, pole_pairs));
...
float BLDCMotor::angle_openloop(float target_angle){
  // get current timestamp
  unsigned long now_us = time_us_64();
  // calculate the sample time from last call
  float Ts = (now_us - open_loop_timestamp) * 1e-6f;
  ...
  if(abs( target_angle - _shaft_angle ) > abs(velocity_limit*Ts)){
    _shaft_angle += _sign(target_angle - _shaft_angle) * abs( velocity_limit )*Ts;
    _shaft_velocity = velocity_limit;
  }else{
    _shaft_angle = target_angle;
    _shaft_velocity = 0;
  }
  ...
  set_phase_voltage(Uq,  0, _calc_electrical_angle(_normalize_angle(_shaft_angle), pole_pairs));
